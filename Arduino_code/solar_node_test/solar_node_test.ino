		...
		if (msr.getStatus() == ASSOCIATED) {
	  	...
	  	// not something we were expecting
                  	if (xbee.getResponse().getApiId() == ZB_TX_STATUS_RESPONSE) {
                                    flashLed(ERROR_LED, 10, 300);
                  }
                  else {
                                                        flashLed(ERROR_LED, 20, 300);
                  }
	  }
